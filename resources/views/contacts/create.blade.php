    ...
    <main class="bg-white flex-1">
      ...
      <div class="bg-gray-100 p-8 md:p-0 md:flex md:items-start md:justify-evenly">
        ...
        <div class="md:w-2/5 md:ml-8 mx-auto md:mx-0 flex flex-col md:items-start">
          ...
          <div class="w-full">
        <form method="POST" action="{{ route('contacts.store') }}" class="max-w-sm mx-auto"> @csrf <label for="website-admin" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Volledige naam</label>
          <div class="flex">
          ...
          <div class="relative">
            ...
            <input type="email" id="email" name="email" value="{{ old('email') }}" class="form-control @error('email') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="robin@student.ehb.be" required autofocus> @error('email') <span class="invalid-feedback" role="alert">
              <strong>{{ $message }}</strong>
            </span> @enderror
          </div>
          <label for="bericht" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white mt-5">Bericht</label>
          <textarea name="bericht" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mb-5" placeholder="Schrijf een bericht..." required>{{ old('bericht') }}</textarea> @error('bericht') <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
          </span> 
          @enderror 
          <button type="submit" class="focus:outline-none text-white bg-teal-500 hover:bg-red-500 focus:ring-4 focus:ring-red-500 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-500 dark:focus:ring-teal-500">Indienen</button>
        </form> 
</div>
